...
flowchart TD
    ...
    subgraph Tool ["RL-Log-Comparator (PySide6 GUI)"]
        Tab1["1. ハイパーパラメータ差分<br>(YAMLツリー比較)"]
        Tab2["2. 画像目視比較<br>(同期パン＆ズーム)"]
        Tab3["3. 数値ログ比較グラフ<br>(CSV重ね合わせプロット)"]
        Tab4["4. 仕様ガイド & エクスポート<br>(DATA_FORMAT.md)"]
    end
